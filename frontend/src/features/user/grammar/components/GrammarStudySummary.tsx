...
interface GrammarStudySummaryProps {
  totalGrammar: number;
  totalSets: number;
  learnedCount: number;
...
export default function GrammarStudySummary({
  totalGrammar,
  totalSets,
  learnedCount,
  notLearnedCount,
  totalTime,
}: GrammarStudySummaryProps) {
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-6">
      <div className="max-w-md w-full bg-white rounded-xl shadow p-6 text-center space-y-4">
        ...
        <p className="text-gray-600">
          Kamu sudah mempelajari <b>{totalGrammar}</b> grammar dalam{" "}
          <b>{totalSets}</b> set.
        </p>
