...
import { fetchGrammarDurationProgress } from "@/features/user/grammar/grammarProgressDurationSlice";
...
const GrammarProgressDuration = () => {
  const theme = chartThemes.grammar;
  const [range, setRange] = useState<"week" | "month" | "year">("week");

  const dispatch = useAppDispatch();

  useEffect(() => {
    dispatch(fetchGrammarDurationProgress(range));
  }, [dispatch, range]);

  
    const { progress, loading } = useAppSelector((state) => state.grammarProgressDuration);
...
export default GrammarProgressDuration;
