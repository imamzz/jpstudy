...
const StartLearning = ({ userId }: StartLearningProps) => {
  ...
  const { grammar, loading, error } = useAppSelector((state) => state.settings);

  // ✅ Ambil setting vocab dari backend
  useEffect(() => {
    if (userId) dispatch(fetchGrammarSetting(userId));
  }, [dispatch, userId]);
  ...
  return (
    <>
        ...
        <div>
          ...
          {loading ? (
            ...
            <p className="text-lg font-semibold text-white">
              Belajar grammar: {grammar.grammar_per_set} soal per sesi
            </p>
          ) : (
        ...
        {!loading && grammar && (
          <div className="flex items-center justify-between">
            <div className="set flex items-center gap-2">
              ...
              <p className="text-md text-white">{grammar.total_set}</p>
            </div>
            <div className="timer flex items-center gap-2">
              <IconTimer className="w-4.5 h-4.5 text-white [stroke-width:2]" />
              <p className="text-md text-white">{grammar.seconds_per_grammar}s</p>
            </div>
            <div className="break flex items-center gap-2">
              <IconTimerPause className="w-5 h-5 text-white [stroke-width:2]" />
              <p className="text-md text-white">{grammar.break_per_set}s</p>
            </div>
            <div className="word flex items-center gap-2">
              <Badge variant={grammar.target_level as BadgeVariant} size="md">
                {grammar.target_level}
              </Badge>
            </div>
          </div>
        )}
      ...
      {/* ⚙️ Modal konfigurasi grammar */}
      <ModalConfig
        isOpen={isOpen}
        onClose={() => setIsOpen(false)}
        userId={userId}
        title="Konfigurasi Grammar"
        description="Atur preferensi belajar grammar"
      />
    </>
  );
};
