...
const StartLearning = ({ userId }: StartLearningProps) => {
  ...
  const { kanji, loading, error } = useAppSelector((state) => state.settings);

  // ✅ Ambil setting kanji dari backend
  useEffect(() => {
    if (userId) dispatch(fetchKanjiSetting(userId));
  }, [dispatch, userId]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    navigate("/kanji/study");
  };
  ...
  return (
    <>
        ...
        <div>
          ...
          {loading ? (
            ...
            <p className="text-lg font-semibold text-white">
              Belajar kanji: {kanji.kanji_per_set} kanji per sesi
            </p>
          ) : (
      ...
      {/* ⚙️ Modal konfigurasi kanji */}
      <ModalConfig
        isOpen={isOpen}
        onClose={() => setIsOpen(false)}
        userId={userId}
        title="Konfigurasi Kanji"
        description="Atur preferensi belajar kanji"
      />
    </>
  );
};
